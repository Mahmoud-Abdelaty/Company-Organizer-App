...
class _PersonalAttendanceState extends State<PersonalAttendanceScreen> {
  ...
  CollectionReference attendanceYesterday = FirebaseFirestore.instance
      .collection('Attendance')
      .doc(DateFormat('MMMM').format(DateTime.now()))
      .collection(DateFormat('dd').format(DateTime.now().subtract(Duration(days: 1))));
  ...
  Future<void> getRecord() async {
    try {
      ...
      DocumentSnapshot snapYesterday = await attendanceYesterday.doc(user['name']).get();

        if(DateFormat('hh a').format(DateTime.now()) == '12 AM'
            || DateFormat('hh a').format(DateTime.now()) == '02 AM'
            || DateFormat('hh a').format(DateTime.now()) == '01 AM'
            || DateFormat('hh a').format(DateTime.now()) == '03 AM'){
          setState(() {
            AppLoginCubit.get(context).checkIn = snapYesterday['Check In'];
            AppLoginCubit.get(context).checkOut = snapYesterday['Check Out'];
          });
        }else{
          setState(() {
      ...
      if(DateFormat('hh a').format(DateTime.now()) == '12 AM'
          ...
          || DateFormat('hh a').format(DateTime.now()) == '03 AM'){
      setState(() {
        AppLoginCubit.get(context).checkIn = AppLoginCubit.get(context).checkIn;
      });
    }
    }
  }
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  ...
                                  //   DateFormat('hh a').format(DateTime.now()) == '12 AM'
                                  //       || DateFormat('hh a').format(DateTime.now()) == '02 AM'
                                  //       || DateFormat('hh a').format(DateTime.now()) == '01 AM'
                                  //       || DateFormat('hh a').format(DateTime.now()) == '03 AM' ?
                                  // AppLoginCubit.get(context).checkIn : AppLoginCubit.get(context).checkIn,
                    ...
                    Stack(
                      children: [
                        Container(
                          ...
                          child: RichText(
                            text: TextSpan(
                              text: DateFormat('hh a').format(DateTime.now()) == '12 AM'
                                  || DateFormat('hh a').format(DateTime.now()) == '01 AM'
                                  || DateFormat('hh a').format(DateTime.now()) == '02 AM'
                                  || DateFormat('hh a').format(DateTime.now()) == '03 AM' ? DateFormat('dd MMMM yyyy').format(DateTime.now().subtract(Duration(days: 1))) : DateFormat('dd MMMM yyyy').format(DateTime.now()),
                              style: TextStyle(
                                fontSize: screenWidth / 15,
                                color: AppCubit.get(context).isDark
                                    ? Colors.white
                              ...
                              builder: (context) {
                                ...
                                return SlideAction(
                                  text: AppLoginCubit.get(context).checkIn == "--/--"
                                      ? translator.translate('slidetoCheckIn') : translator.translate('slidetoCheck Out'),
                                  textStyle: TextStyle(
                                    color: AppCubit.get(context).isDark
                                        ? Colors.white
                                        : HexColor('333739'),
                                                        ...
                                                        myButton(
                                                            ...
                                                            onPressed:
                                                              ...
                                                              if (formKey
                                                                  ...
                                                                  .validate()) {
                                                                if (code == user['code']) {
                                                                  if (AppLoginCubit.get(context).checkIn == '--/--') {
                                                                      Navigator.pop(context);
                                                                      AppLoginCubit.get(context).changeCheckIn();
                                                                      attend();
                                                                  } else if(DateFormat('hh a').format(DateTime.now()) == '12 AM'
                                                                         || DateFormat('hh a').format(DateTime.now()) == '01 AM'
                                                                         || DateFormat('hh a').format(DateTime.now()) == '02 AM'
                                                                         || DateFormat('hh a').format(DateTime.now()) == '03 AM'){
                                                                      Navigator.pop(context);
                                                                      AppLoginCubit.get(context).changeCheckOut();
                                                                      attend4();
                                                                    } else {
                                                                    Navigator.pop(context);
                                                                    AppLoginCubit.get(context).changeCheckOut();
                                                                  ...
                                                                  Navigator.pop(context);
                                                                  showSnackBar(context, 'Wrong Code', Colors.red);
                                                                }
                                                              }
                                                            }),
                                                      ],
  ...
  Future<void> attend4() async {
    DocumentSnapshot user = await FirebaseFirestore.instance
        .collection('Users')
        .doc(FirebaseAuth.instance.currentUser?.uid)
        .get();

    await attendanceYesterday.doc(user['name']).update({
      'Check Out': AppLoginCubit.get(context).checkOut,
